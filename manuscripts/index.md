...
元々の JavaScript の実装は Saga にジェネレーター関数を用いていますが、
...
enum は型の比較が難しい、struct は実装の過程で継承を利用したいので難しいです
...
無限ループ！？という感覚は正常です。
ループの中で Action が発行されるまで待ち、
それが発行されたら Saga を実行するという処理を繰り返します。
